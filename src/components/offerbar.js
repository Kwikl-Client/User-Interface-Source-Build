...
const SalesCountdownTimer = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const emailCheckResponse = await axios.post('http://18.207.152.156:7000/customer/checkUser', { email });
      ...
      const paymentResponse = await axios.get(
        `http://18.207.152.156:7000/payment/createOfferPaymentIntent/?email=${email}&name=${name}`
      );
