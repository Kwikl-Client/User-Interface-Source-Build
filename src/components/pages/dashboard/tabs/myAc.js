...
export default function MyAc({ customerDetails, setCustomerDetails }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.patch("http://172.31.28.17:7000/customer/editCustomerDetails", payload, config);
  ...
  const handleReviewSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`http://172.31.28.17:7000/customer/usersReview/${customerDetails._id}`, {
        starRating,
        reviewText,
        profession,
      });
    ...
    <div className='accounts-tab' style={{ flexDirection: 'column' }}>
      ...
      {isReviewFormVisible && (
        <form className="review-form" onSubmit={handleReviewSubmit}>
          ...
          />
          {reminderMessage && <p style={{color: 'red',fontSize:'10px' }}>{reminderMessage}</p>}
