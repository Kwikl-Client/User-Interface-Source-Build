...
export default function MyAc({ customerDetails, setCustomerDetails }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.patch("http://18.209.7.74:7000/customer/editCustomerDetails", payload, config);
  ...
  const handleReviewSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`http://18.209.7.74:7000/customer/usersReview/${customerDetails._id}`, {
        starRating,
        reviewText,
        profession,
      });
